...
class playersim:
    ...
    def playerP(self,nthinnings,thisinnings, bowlingteam,simteams,simteamstats,thisover,hometeam):
        ...
        wicketprobs ={
        self.batters[0]:
        simteamstats.ballresult['Out/NotOut'].loc[(simteamstats.ballresult["Batter"]==self.batters[0])&(simteamstats.ballresult["Innings Type"]=="Batting")&(simteamstats.ballresult["Balls Faced"]<(self.battersstats[self.batters[0]]+6))].value_counts(normalize=True,sort=False)
        ,
        self.batters[1]:
        ...
        # what about debutant bowlers?
        battersprobs ={
        self.batters[0]:
        simteamstats.ballresult["Batter Score"].loc[(simteamstats.ballresult["Batter"]==self.batters[0])&(simteamstats.ballresult["Balls Faced"]<(self.battersstats[self.batters[0]]+6))].value_counts(normalize=True,sort=False)
        ,
        self.batters[1]:
        ...
        scoreP={
            self.batters[0]: (battersprobs[self.batters[0]]*batadv).add(
            simteamstats.ballresult['Batter Score'].loc[(simteamstats.ballresult["Bowler"].isin(simteams[bowlingteam]["bowlers"]))&(simteamstats.ballresult["Innings Type"]=="Bowling")&(simteamstats.ballresult["Innings Ball"]>(thisover))&(simteamstats.ballresult["Innings Ball"]<(thisover+1))].value_counts(normalize=True,sort=False)*bowladv,fill_value=0)
            ,
            self.batters[1]: (battersprobs[self.batters[1]]*batadv).add(
            simteamstats.ballresult['Batter Score'].loc[(simteamstats.ballresult["Bowler"].isin(simteams[bowlingteam]["bowlers"]))&(simteamstats.ballresult["Innings Type"]=="Bowling")&(simteamstats.ballresult["Innings Ball"]>(thisover))&(simteamstats.ballresult["Innings Ball"]<(thisover+1))].value_counts(normalize=True,sort=False)*bowladv,fill_value=0)
    ...
    def testmatch(self,rng,statsmatchtype,inningsorder,rain,simteams,simteamstats,matchscore,hometeam,endover):
        ...
        for nthinnings, thisinnings in enumerate(inningsorder):
            ...
            while matchover<450:
                ...
                extrasP, fieldingextrasP = tm.extras(self,nthinnings,thisinnings, bowlingteam,simteams,simteamstats,thisover)
                if len(extrasP)==0 or sum(extrasP)<0.99:
                    extrasP=playersim.extrasfallbackplayersP(self,nthinnings,thisinnings,bowlingteam,simteams,simteamstats,thisover,hometeam,)
